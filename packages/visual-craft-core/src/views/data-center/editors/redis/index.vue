<template>
  <div class="redis-editor redis-database-editor">
...
import RedisStepManager from './components/redis-step-manager.vue';
import RedisEditorHeader from './components/redis-editor-header.vue';
...
const config = reactive({
  ...
  name: props.initialData?.name || '新建 Redis 操作',
  type: 'redis',
...
const currentStep = computed(() => {
  ...
  if (currentStepIndex.value === -2) return { name: 'Redis 连接配置', type: 'config' };
...
:is(#visual-craft-core, #visual-craft-core-project) {
  .redis-database-editor {
    /* Dedicated Redis Identity (Vibrant Red) */
    --redis-color-primary: #ef4444;
    --redis-color-primary-hover: #dc2626;
    --redis-color-primary-light: rgba(239, 68, 68, 0.12);

    --redis-color-accent-lua: #f59e0b;
    --redis-color-accent-lua-light: rgba(245, 158, 11, 0.1);

    /* Local system-blue overrides — only affects components INSIDE this editor */
    --theme-color-blue-700: var(--redis-color-primary);
    --theme-color-blue-800: var(--redis-color-primary-hover);
    --db-editor-color-primary: var(--redis-color-primary);
    --db-editor-color-primary-light: var(--redis-color-primary-light);
