...
import type { AgentContext, IAgentResponse, IChatImageAttachment, AgentRole } from '../../types';
...
export function useOrchestrator() {
  ...
  const resolveApplyStrategy = (agent?: AgentRole, nodeConfig?: Record<string, unknown>): 'manual' | 'auto' => {
  ...
  const process = async (
    ...
    try {
      ...
      const result = await engine.execute(input, context as unknown as AgentContext,
        (nodeId, partial) => {
