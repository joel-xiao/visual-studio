...
// 选择图层
const onSelect = (item: PanelLayerItemData): void => {
  emit('select', item);
};
...
// 切换隐藏
const onToggleHide = (item: PanelLayerItemData) => {
  nodeContext.layer.setRecursiveProperty(item.id, 'hide', !item.data?.hide);
};

// 切换锁定
const onToggleLock = (item: PanelLayerItemData) => {
  nodeContext.layer.setRecursiveProperty(item.id, 'lock', !item.data?.lock);
};

// 判断是否继承隐藏
const isInheritedHide = (item: PanelLayerItemData): boolean => {
...
// 判断是否继承锁定
const isInheritedLock = (item: PanelLayerItemData): boolean => {
  if (!item.parentId || item.parentId === 'root') return false;
  const parent = nodeContext.getNodeMap().get(item.parentId);
...
const onDrop = (e: DragEvent, item: PanelLayerItemData) => {
  ...
  if (sourceId && dropPos.value && sourceId !== item.id) {
    nodeContext.layer.sortNode(sourceId, item.id, dropPos.value);
  }
  dragOverId.value = null;
  dropPos.value = null;
};
...
<template>
  <div v-for="item in data" :key="item.id" class="layer-item" :class="{ 'layer-item_check': item.select }">
    <div 
      ...
      @contextmenu.stop.prevent="showNodeMenu($event, item.id, nodeContext, componentContext, 1)"
    >
      ...
      <div v-if="item.handle !== false" class="layer-item-handle">
        ...
        />
        <span v-else-if="isInheritedLock(item)" class="inherited-dot"></span>
        ...
        />
        <span v-else-if="isInheritedHide(item)" class="inherited-dot"></span>
      </div>
    </div>
  ...
  .layer-item {
    ...
    .layer-item-nav {
      ...
      &.drop-before::before {
        ...
        z-index: 100;
        box-shadow: 0 0 4px var(--db-editor-color-primary-glow);
      }
      ...
      &.drop-before::after {
        ...
        width: 10px;
        height: 10px;
        border: 2px solid var(--db-editor-color-primary);
        border-radius: 50%;
      ...
      &.drop-inside {
        outline: 2px solid var(--db-editor-color-primary);
        outline-offset: -2px;
        background: var(--db-editor-color-primary-light) !important;
        z-index: 50;
      }
      ...
      .layer-item-handle {
        ...
        .lock-btn,
        .hide-btn {
          width: var(--db-layer-icon-size) !important;
          height: var(--db-layer-icon-size) !important;
          color: var(--theme-color-tran-20);
          opacity: var(--db-layer-icon-opacity);
          ...
          &.is-active {
            width: var(--db-layer-icon-size-active) !important;
            height: var(--db-layer-icon-size-active) !important;
            color: var(--theme-color-tran-50);
        ...
        .inherited-dot {
          width: var(--db-layer-dot-size);
          height: var(--db-layer-dot-size);
          background: var(--theme-color-tran-15);
          border-radius: 50%;
        }
      }
