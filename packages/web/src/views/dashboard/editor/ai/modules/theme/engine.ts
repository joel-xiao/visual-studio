...
export class ThemeEngine implements IAgent {
  role: AgentRole = 'theme-engine';
  name = 'Theme Engine';
...
如果需要自定义颜色: {"content":"自定义主题描述","type":"theme-selection","data":{"theme":"custom-name","colors":["#5470c6","#91cc75","#fac858"]}}`,
      ...
      if (jsonMatch) {
        ...
        return this.checkContextForCharts({ content: result.content || '已应用主题', type: 'theme-selection', data: result.data }, context);
      }
  ...
  // Helper to add handoff if charts exist
  private checkContextForCharts(result: any, context: any) {
    const previousNodes = context?.previousAgentData?.nodes || [];
    const hasCharts = previousNodes.some((n: any) => n.component && (n.component.includes('ECHARTS') || n.component.includes('CHART')));
